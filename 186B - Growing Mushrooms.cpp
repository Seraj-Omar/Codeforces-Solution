...
bool Seraj(pair<double,int> &a , pair<double , int> &b)
{
    if (a.first == b.first) {
...
void solve()
{
    ...
    {
        cin>>a>>b;
        double before= min(a,b),after= max(a,b);
        v.emplace_back(max(((after*t1)-(after*t1*(k/100)))+(before*t2),((before*t1)-(before*t1*(k/100)))+(after*t2)),i);
    }
    sort(all(v),Seraj);
    for(auto i:v)
        cout<<i.second<<' '<<i.first<<endd;
    SEEYAH
}
int main()
{
