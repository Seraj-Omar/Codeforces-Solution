...
{
    ll n,m,k;cin>>n>>m>>k;
    vector<vector<pair<ll,pair<ll,bool>>>> v(n+1);
    loop(0,m)
    {
        ll x,y,w;cin>>x>>y>>w;
        v[x].push_back({w,{y,false}});
        v[y].push_back({w,{x,false}});
    }
    vector<ll> visited(n+1),value(n+1),routes(n+1);
    priority_queue<pair<ll,pair<ll,bool>>,vector<pair<ll,pair<ll,bool>>>,greater<>>p;
    set<ll>s;

    ll opened=k,all=k;
    while(k--)
    {
        ll node,cost;cin>>node>>cost;
        if(s.find(node)!=s.end()) {
            routes[node] = min(routes[node], cost);
            opened--;
        }
        else
        {
    ...
    for(auto i:s)
    {
        v[i].push_back({routes[i],{1, true}});
        v[1].push_back({routes[i],{i, true}});
    }

    p.push({0,{1, false}});
    while(!p.empty())
    {
        ll cost=p.top().first,node=p.top().second.first;
        bool route=p.top().second.second;
        ...
        visited[node]=1;
        if(s.find(node)!=s.end()&&!route)
            opened--;
        value[node]=cost;

        for(auto i:v[node])
        {
            if(!visited[i.second.first])
                p.emplace(cost+i.first,i.second);
        }
    }
    cout<<all-opened<<'\n';
}
