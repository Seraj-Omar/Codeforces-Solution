...
void solve()
{
    ...
    vpp v(n+1);
    vp val(n+1,{-1,0});//to take the weight and the parent of the node

    loop(0,m)
    {
        ll x,y,w;cin>>x>>y>>w;
        v[x].emplace_back(w,y);
        v[y].emplace_back(w,x);
    }

    priority_queue<pair<ll,pair<ll,ll>>,vector<pair<ll,pair<ll,ll>>>,greater<>>p;
    p.push({0,{1,0}});
    while(!p.empty())
    {
        ll cost=p.top().first,node=p.top().second.first,parent=p.top().second.second;
        p.pop();
        if(val[node].first!=-1)
            continue;
        val[node]={cost,parent};
        for(auto j:v[node])
        {
            p.push({cost + j.first, {j.second,node}});
        }
    }
